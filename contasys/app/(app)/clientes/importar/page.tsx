...
async function importarClientes(_prev: ImportState | null, formData: FormData): Promise<ImportState> {
  ...
  const required = ["identificacion", "nombre"];
  for (const r of required) {
    if (!headers.includes(r)) return { error: `Falta la columna requerida: "${r}"` };
  }
  ...
  if (clientes.length === 0) return { error: "No se encontraron registros válidos" };
...
export default function ImportarClientesPage() {
  const [state, formAction, pending] = useActionState<ImportState | null, FormData>(importarClientes, null);
    ...
    return (
      <div className="max-w-lg mx-auto mt-20 text-center space-y-4">
        ...
        <p className="text-muted-foreground">
          Se importaron <strong>{state.count}</strong> cliente{state.count !== 1 ? "s" : ""} correctamente.
        </p>
        <div className="flex gap-3 justify-center">
          <Link href="/clientes" className="inline-flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90">
            Ver Clientes
          </Link>
  ...
  return (
    <div className="max-w-lg mx-auto mt-10 space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Sube un archivo CSV con los datos de tus clientes.</p>
      </div>
      ...
      <div className="rounded-lg border p-4 text-sm text-muted-foreground space-y-1">
        ...
        <code className="text-xs block">identificacion,nombre,razon_social,email,telefono,direccion,ciudad,provincia,etiquetas</code>
        <code className="text-xs block">1234567890,Juan Pérez,,juan@mail.com,0999999999,Av. Siempre Viva,Quito,Pichincha,cliente vip</code>
        <Link
          href="/api/clientes/exportar?formato=ejemplo"
